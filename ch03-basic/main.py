...
@cl.set_chat_profiles  # ty: ignore[no-matching-overload]
...
async def count_clicks(action: cl.Action) -> None:
    ...
    await action.send(for_id=action.forId)  # ty: ignore[invalid-argument-type]


@cl.set_starters  # ty: ignore[no-matching-overload]
...
async def on_message(message: cl.Message) -> None:  # noqa: C901
    ...
    match message.content.lower():
        ...
        case "action":
            ...
            await cl.Message(
                content="Let's click some buttons!",
                actions=actions,
            ).send()
        ...
        case "element":
            ...
            await cl.Message(
                content="Here are some elements:",
                elements=elements,
            ).send()
        case "chat profile":
            await cl.Message(
